...
public class BinSort {
    ...
    public static void binSort(Elem[] arr, int maxValue) {
        ...
        for (int j = 0; j < bins.length; j++) {
            // This approach uses an iterator to go over the linked list
            ...
            /* // Instead of the iterator, we could have used a "for each" loop to go over the linked list:
            for (Elem elem: bins[j]) {
                arr[count] = elem;
                count++;
            }
             */
